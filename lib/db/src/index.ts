...
// Each statement is run individually — pg does not support multiple
// semicolon-separated statements in a single client.query() call.
const COLUMN_FIX_STATEMENTS = [
  // users table
 ...
 * Each ALTER TABLE statement is executed individually (pg does not support
 * multiple statements in one query call). All use IF NOT EXISTS — idempotent.
 *
 * This bypasses the Drizzle migration tracker entirely, guaranteeing the
 * columns exist even if the journal previously recorded them as applied
 * without actually executing the SQL.
 */
export async function applyColumnFixes(): Promise<void> {
  const client = await pool.connect();
  try {
    for (const sql of COLUMN_FIX_STATEMENTS) {
