...
export default function AdminSessionsPage() {
  ...
  const fetchSessions = async () => {
    try {
      setIsFetching(true);
      const res = await fetch("/api/admin/license-keys/active-streaming", {
  ...
  async function handleTerminate(sessionId: string) {
    ...
    try {
      const res = await fetch(`/api/admin/license-keys/${sessionId}/terminate`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        alert(body.error ?? "Failed to terminate session");
      } else {
        await fetchSessions();
      }
    } catch {
      alert("Network error — could not terminate session");
    } finally {
      setTerminating(prev => ({ ...prev, [sessionId]: false }));
    }
  }
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-background/50">
                    ...
                    <th className="text-left p-4 text-muted-foreground font-medium">Minutes Allocated</th>
                    <th className="text-left p-4 text-muted-foreground font-medium">Minutes Remaining</th>
                ...
                <tbody>
                  {sessions.map((s) => (
                    <tr key={s.sessionId} data-testid={`session-row-${s.sessionId}`} className="border-b border-border last:border-0 hover:bg-accent/5">
                      <td className="p-4">
                        <p className="font-medium text-foreground font-mono text-xs">{s.key.slice(0, 8)}…{s.key.slice(-4)}</p>
                      </td>
