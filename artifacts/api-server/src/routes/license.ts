...
router.post("/validate", async (req, res) => {
  try {
    ...
    // If this license was generated by a sub-admin, verify that sub-admin
    // has an API key assigned. Unassigned sub-admins cannot authorize streaming.
    if ((license as any).createdBySubAdminId) {
      try {
        ...
        const saCheck = await db.execute(
          `SELECT sub_admin_assigned_decart_key_id FROM users WHERE id = ${(license as any).createdBySubAdminId} AND is_sub_admin = 1`
        );
        const saRow = saCheck.rows[0] as any;
        if (!saRow || !saRow.sub_admin_assigned_decart_key_id) {
          return res.json({ valid: false, error: "Streaming not authorized — contact your administrator" });
        }
      } catch { /* non-fatal: skip if column not yet migrated */ }
    }
